...
class CHTML
{
     ...
     * @return HtmlString
     */
    public static function statusEnabledToggle(Model $model): HtmlString
    {
        $HTML = "<input class='toggle-class' type='checkbox' data-onstyle='success' data-offstyle='danger' ";
        $HTML .=" data-toggle='toggle' data-size='small' data-model='". get_class($model) . "' data-id='" .$model->id. "' data-field='enabled' ";
        $HTML .= "data-on='" . $options['on'] . "' data-off='" . $options['off'] . "'";
        if (is_null($current_value)) {
            $HTML .= ($options['on'] == $model->$field) ? " checked" : "";
        } else {
